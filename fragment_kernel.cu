...
template <typename T>
DevicePrimitive<T> PrimitiveToDevice(const Primitive* scene);

template <>
DevicePrimitive<Sphere> PrimitiveToDevice(const Primitive* scene) {
    return DevicePrimitive<Sphere>(scene->getLoc(), scene->getRot(), scene->getScale(), ((Sphere*)scene)->getRadius());
}
template <>
DevicePrimitive<Cube> PrimitiveToDevice(const Primitive* scene) {
    return DevicePrimitive<Cube>(scene->getLoc(), scene->getRot(), scene->getScale());
}
template <>
DevicePrimitive<Mandelbulb> PrimitiveToDevice(const Primitive* scene) {
    return DevicePrimitive<Mandelbulb>(scene->getLoc(), scene->getRot(), scene->getScale(), ((Mandelbulb*)scene)->getIterations(), ((Mandelbulb*)scene)->getExponent());
}
...
void launchFragment(cudaSurfaceObject_t surf,unsigned int width, unsigned int height, float time, const Primitive* scene) {
    ...
    switch (scene->getType()) {
        case PrimitiveType::SPHERE: {
            DevicePrimitive<Sphere>* DP;
            DevicePrimitive<Sphere> Conversion = PrimitiveToDevice<Sphere>(scene);
            cudaMalloc(&DP, sizeof(DevicePrimitive<Sphere>));
            cudaMemcpy(DP, &Conversion, sizeof(DevicePrimitive<Sphere>), cudaMemcpyHostToDevice);
            FragmentKernel<<<GridDim, ThreadsPerBlock>>>(surf,width, height,time,DP);
        }
            break;

        case PrimitiveType::CUBE: {
            DevicePrimitive<Cube>* DP;
            DevicePrimitive<Cube> Conversion = PrimitiveToDevice<Cube>(scene);
        ...
        case PrimitiveType::MANDELBROT: {
            DevicePrimitive<Mandelbulb>* DP;
            DevicePrimitive<Mandelbulb> Conversion = PrimitiveToDevice<Mandelbulb>(scene);
            //printf("%d \n", Conversion.mandelbulb.iterations);
            cudaMalloc(&DP, sizeof(DevicePrimitive<Mandelbulb>));
            cudaMemcpy(DP, &Conversion, sizeof(DevicePrimitive<Mandelbulb>), cudaMemcpyHostToDevice);
            FragmentKernel<<<GridDim, ThreadsPerBlock>>>(surf,width, height,time,DP);
        }
